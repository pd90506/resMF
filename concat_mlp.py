...
from tensorflow.keras.layers import Input, Flatten, Embedding, Dense, Concatenate
from tensorflow.keras.regularizers import l2
from mlp import mlp_model
from utils import init_normal


def concat_mlp_model(num_items, factor=10, layers = [20,10], emb_reg=0, reg_layers=[0,0]):
    ...
    vector = mlp(latent)
    concated = Concatenate()([latent, vector])
    model = Model(inputs=[item_input], outputs=concated)
    return model


if __name__ == '__main__':
    NUM_ITEMS = 1000
    model = concat_mlp_model(NUM_ITEMS)
